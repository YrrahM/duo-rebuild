...
import { HOME_MESSAGES as MESSAGES, Locale } from '@/messages/home'; // <- import from lowercase 'messages'
...
export default function HomeClient({ locale }: { locale: Locale }) {
  ...
  return (
    <section id="home" aria-label={t.aria} className="relative w-full overflow-hidden"
      style={{ height: '100svh', backgroundColor: '#0b1220', isolation: 'isolate' }}>
      <div className="absolute inset-0 z-0">
        <Image src={heroImg} alt={t.alt} fill priority quality={90} placeholder="blur"
               className="object-cover object-center" sizes="100vw" />
      </div>
      <div className="absolute inset-0 z-10" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }} />
      <div className="relative z-20 text-left max-w-3xl px-4 md:px-8"
           style={{ paddingTop: '10vh', paddingLeft: '1cm' }}>
        ...
        <Link href={featuresHref} className="inline-block font-bold py-3 px-6 rounded-xl text-lg transition duration-300 mt-6"
